...
const RowDetails_Factory = [
  ...
  {name: "rebated / defect allowance ",              id:"rebated"}, 
...
const Catorgies = ["Fitted Sheet", "Apparel", "Towels", "Blanket","Mattress Cover"];
...
function addColumn_CMTCost(items){
  ...
  for (let index = 0; index < items.length; index++) {
    ...
    CMTCostTable.innerHTML += `<tr>
      <td>${(index+1)}</td>
      <td>${temp.name} </td>
      <td><input type="number" onchange="onChange_CMTCost()" id="${temp.id}_Piece" name="${temp.id}_CMT" placeholder="Enter Cost per Pack" min="0" step="0.01" required /></td>
    </tr>`;
  }
}
...
function onChange_NetCost(){
  ...
  getDocument("TotalNetCost_Piece").innerHTML = parseFloat(totalPiece).toFixed(2);
...
function onChangeCurrenct() {
  ...
  if (IsCurrency) {
    let template = `<span class="doubleInput">
    ...
    getDocument("IsCurrency").parentElement.parentElement.lastElementChild.innerHTML = template;

  }else{
    getDocument("IsCurrency").parentElement.parentElement.lastElementChild.innerHTML = "";
  }
}
